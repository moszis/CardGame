        function drawCard(cardID){
                ...
                if (slot !== null) {
                    var card = PIXI.Sprite.fromImage("assets/images/cards/Card2Frame.png");
			...
			this.data = null;
                        ...
                        if(mouseAtSlot >= 0 && slotAllowed(mouseAtSlot)){
                           if(cardSlots[mouseAtSlot].cardID !== -1){  
                               ...
                               updateCardRemMoves(this, getDistanceBetweenSlots(playerCards[this.id].slotID, mouseAtSlot));
		...
		card.mousemove = card.touchmove = function(data)
		{
			if(this.dragging)
			{
                ...
                card.mouseout = card.touchend = function(data){
                    ...
                    var statNumberFont = cardFrameOriginalHeight / 10;
                    
                    var atkNumber     = new PIXI.Text(playerCards[cardID].attack);
                    var hpNumber      = new PIXI.Text(playerCards[cardID].health);
                    var actionPoints  = new PIXI.Text(playerCards[cardID].speed);
                    
            
                    //TOP LEFT
                    atkNumber.height = (cardFrameOriginalHeight / 10) * 0.25;
                    atkNumber.width  = (cardFrameOriginalWidth / 10) * 0.25;  
                    atkNumber.position.x = 50 ;
                    atkNumber.position.y = 10;
                    
                    //TOP RIGHT                  
                    hpNumber.height = (cardFrameOriginalHeight / 10) * 0.25;
                    hpNumber.width  = (cardFrameOriginalWidth / 10) * 0.25;  
                    hpNumber.position.x = cardFrameOriginalWidth - 90;
                    hpNumber.position.y = 10;
                    
                    //BOTTOM LEFT
                    actionPoints.height = (cardFrameOriginalHeight / 10) * 0.25;
                    actionPoints.width  = (cardFrameOriginalWidth / 10) * 0.25; 
                    actionPoints.position.x = 50;
                    actionPoints.position.y = cardFrameOriginalHeight - 110;
                    
                    atkNumber.setStyle({font:""+statNumberFont+"px Arial Black", fill:"white"});
                    hpNumber.setStyle({font:""+statNumberFont+"px Arial Black", fill:"white"});
                    actionPoints.setStyle({font:""+statNumberFont+"px Arial Black", fill:"white"});
                    ...
                    var cardImage = PIXI.Sprite.fromImage(playerCards[cardID].imgSrc);
                    ...
                    card.addChild(atkNumber);
                    card.addChild(hpNumber);
                    card.addChild(actionPoints);
